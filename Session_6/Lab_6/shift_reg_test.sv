...
class shift_reg_test extends uvm_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    reset_seq.start(env.agt.sequnecer);
    `uvm_info("run_phase", "Reset Dessserted", UVM_MEDIUM);
  
    `uvm_info("run_phase", "Main Seq Asserted", UVM_MEDIUM);
    main_seq.start(env.agt.sequnecer);
    `uvm_info("run_phase", "Main Seq Asserted", UVM_MEDIUM);
